...
const Clientes = ({ navigate }) => {
  ...
  const handleSave = async () => {
    if (!nome || !usuario || !email || !senha || !confirmarSenha || !telefone) {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.inputGroup}>
          ...
          <input id="telefone" type="tel" style={styles.input} placeholder="(XX) XXXXX-XXXX" value={telefone} onChange={(e) => setTelefone(e.target.value)} />
        </div>

        <button style={styles.button} onClick={handleSave}>Salvar Cliente</button>
        <button style={{ ...styles.button, ...styles.backButton }} onClick={() => navigate('Menu')}>Voltar ao Menu</button>
      </div>
    </div>
  );
};
